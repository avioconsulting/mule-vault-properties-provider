...
public class VaultConfigurationPropertiesProviderFactory implements ConfigurationPropertiesProviderFactory {
   ...
   *
   * @param parameters
   *            The parameters read from the Mule config file
   * @return a fully configured {@link Vault} object
   */
  private AbstractConnectionProvider getVaultConnectionProvider(ConfigurationParameters parameters,
      VaultConfig vaultConfig) throws ConnectionException {
    ...
    /*
     * Issue #12:
     * https://github.com/avioconsulting/mule-vault-properties-provider/issues/12
     * Mule 4.4 added an expiration-policy element that is added automatically, so
     * we need to
    ...
    for (int i = 0; i < parameters.getComplexConfigurationParameters().size(); i++) {
      ...
      if (namespace.equals(VaultPropertiesProviderExtension.VAULT_PROPERTIES_PROVIDER.getNamespace())) {
        ...
        ConfigurationParameters configurationParameters = parameters.getComplexConfigurationParameters().get(i)
            .getSecond();
