...
public abstract class AbstractConnection implements VaultConnection {
  ...
  /**
   * Construct {@link SslConfig} given the tls-context element for TLS connections
   * to Vault
   *
   * @param tlsContext
   *            properties in the tls-context element
   ...
   * @throws IOException
   */
  public SslConfig getVaultSSLConfig(TlsContext tlsContext)
    ...
    if (tlsContext != null) {
      ...
      if (tlsContext.isKeyStoreConfigured()) {
        ssl = ssl.keyStore(tlsContext.getKeyStoreConfig().getKeyStore(),
            tlsContext.getKeyStoreConfig().getPassword());
      }
    }
    return ssl;
  }
